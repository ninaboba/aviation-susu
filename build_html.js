...
<body class="bg-cockpit-950 text-slate-100 min-h-screen font-sans antialiased grid-pattern selection:bg-cyan-500 selection:text-white flex flex-col justify-between transition-colors duration-200">
  ...
  <header class="sticky top-0 z-40 border-b border-cockpit-border glass-panel transition-colors duration-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      <!-- Logo & Subject Dropdown -->
      <div class="flex items-center space-x-3 cursor-pointer" onclick="app.goHome()">
        <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-cyan-600 to-blue-500 flex items-center justify-center shadow-glow-cyan text-white shrink-0">
          <i data-lucide="plane" class="w-5 h-5"></i>
        </div>
        <div class="flex flex-col justify-center">
          <div class="flex items-center space-x-2">
            <span id="headerSubjectCode" class="font-mono font-bold text-base tracking-wider bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-300 hidden sm:inline">081</span>
            <select id="headerSubjectSelect" onchange="app.handleSubjectChange(this.value)" onclick="event.stopPropagation()" class="text-xs sm:text-sm font-mono font-bold px-2 py-1 rounded-lg bg-cockpit-850 hover:bg-cockpit-800 text-cyan-300 border border-cockpit-border cursor-pointer focus:outline-none focus:border-cyan-400 transition max-w-[200px] sm:max-w-none truncate">
      ...
      <div class="flex items-center space-x-1.5 sm:space-x-2.5">
        ...
        <div id="headerOverflowMenuWrap" class="relative">
          <button id="headerOverflowBtn" onclick="app.toggleHeaderMenu()" class="p-2 rounded-lg bg-cockpit-850 hover:bg-cockpit-800 text-slate-300 hover:text-cyan-400 border border-cockpit-border transition text-xs flex items-center justify-center relative" title="More Options">
            <i data-lucide="more-horizontal" class="w-4 h-4"></i>
            <span id="headerMenuDot" class="hidden absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-rose-500 animate-pulse border border-cockpit-900"></span>
          </button>

          <!-- Dropdown Menu -->
          <div id="headerOverflowDropdown" class="hidden absolute right-0 mt-2 w-48 py-1.5 rounded-xl glass-panel border border-cockpit-border shadow-xl z-50 text-xs font-mono space-y-0.5">
            <!-- Audio Toggle -->
            <button id="soundToggleBtn" onclick="app.toggleSound(); app.closeHeaderMenu();" class="w-full px-3 py-2 text-left hover:bg-cockpit-800/80 text-slate-300 hover:text-cyan-400 flex items-center space-x-2 transition">
              <i id="soundIcon" data-lucide="volume-2" class="w-4 h-4 text-cyan-400"></i>
              <span>Sound Effects</span>
            </button>
            <div class="border-t border-cockpit-border/50 my-1"></div>
            <!-- Mistakes Manager -->
            <button onclick="app.openMistakesManager(); app.closeHeaderMenu();" class="w-full px-3 py-2 text-left hover:bg-cockpit-800/80 text-slate-300 flex items-center justify-between transition">
              <div class="flex items-center space-x-2">
            ...
            <!-- Bookmarks Manager -->
            <button onclick="app.openBookmarksManager(); app.closeHeaderMenu();" class="w-full px-3 py-2 text-left hover:bg-cockpit-800/80 text-slate-300 flex items-center justify-between transition">
              <div class="flex items-center space-x-2">
    ...
    <section id="viewSetup" class="space-y-6">
      ...
      <div class="relative overflow-hidden rounded-2xl glass-panel p-6 sm:p-8 border border-cockpit-border">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div class="space-y-2">
            ...
            <h1 id="heroSubjectTitle" class="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">
              Principles of Flight — Examination Trainer
            </h1>
            <p id="heroSubjectDescription" class="text-sm text-slate-300 max-w-2xl leading-relaxed">
              ชุดข้อสอบ 200 ข้อพร้อมระบบสุ่มคำตอบแบบ <span class="text-cyan-400 font-medium">Fisher-Yates Shuffle</span>, เฉลยรายละเอียดแยกตามบทเรียน, ระบบจับเวลา และการวิเคราะห์ผลลัพธ์เพื่อเตรียมสอบอย่างมั่นใจ
            </p>
          </div>
          ...
          <div class="grid grid-cols-3 gap-3 shrink-0">
            <div class="bg-cockpit-850/80 p-3.5 rounded-xl border border-cockpit-border text-center">
              ...
              <span id="heroBankTotal" class="text-xl font-bold font-mono text-cyan-400">200</span>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <!-- 0. SUBJECT SELECTION CARD -->
          <div class="glass-panel p-6 rounded-2xl border border-cockpit-border space-y-3">
            <label class="text-sm font-semibold text-slate-200 flex items-center justify-between">
              <span class="flex items-center space-x-2">
                ...
                <span>เลือกรายวิชาที่ต้องการฝึกซ้อม (Select Subject)</span>
              </span>
            </label>
            <select id="dashboardSubjectSelect" onchange="app.handleSubjectChange(this.value)" class="w-full bg-cockpit-850 hover:bg-cockpit-800 text-cyan-300 font-mono font-bold text-sm border border-cockpit-border rounded-xl p-3 focus:outline-none focus:border-cyan-400 transition cursor-pointer">
              <!-- Rendered dynamically -->
            </select>
          </div>
    ...
    <section id="viewExam" class="hidden space-y-6">
      ...
      <div class="glass-panel p-6 sm:p-8 rounded-2xl border border-cockpit-border relative space-y-6">
        ...
        <div id="explanationBox" class="hidden rounded-xl p-5 bg-cockpit-900 border border-cyan-500/40 space-y-3 animate-fadeIn">
          ...
          <div class="pt-3 border-t border-cockpit-border/60 flex flex-wrap items-center justify-between gap-3 text-xs font-mono text-slate-400">
            ...
            <button id="btnNextQBottom" onclick="app.nextQuestion()" class="hidden ml-auto px-5 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-bold text-xs shadow-glow-cyan transition flex items-center space-x-1.5">
              <span id="btnNextQBottomLabel">Next</span>
